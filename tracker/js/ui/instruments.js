...
export default class InstrumentUI {
  ...
  updateInstrumentList() {
    ...
    for (var idx = 0; idx < song.waves.length; idx++) {
      ...
      for (var i = 0; i < song.wave_instruments.length; i++)
        if (song.wave_instruments[i].wave_index == idx) name += ` ${song.wave_instruments[i].name}`;
      node.innerText = name;
      node.value = idx;
    ...
    if (i instanceof DutyInstrument) {
      ...
      for (var n = 0; n < 128; n++) {
        ...
        if (i.frequency_sweep_time > 0) {
          if (i.frequency_sweep_shift < 0) value -= diff / i.frequency_sweep_time;
  ...
  getSelectedInstrument() {
    ...
    if (option.parentElement.label == 'Duty') return song.duty_instruments[option.value];
    if (option.parentElement.label == 'Wave') return song.wave_instruments[option.value];
    if (option.parentElement.label == 'Noise') return song.noise_instruments[option.value];
    return null;
  }
}
