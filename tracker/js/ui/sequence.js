...
export default class SequenceUI {
  ...
  constructor() {
    ...
    document.getElementById('sequence_del').onclick = (e) => {
      ...
      if (idx == song.sequence.length) idx -= 1;
      this.setCurrentSequenceIndex(idx);
      ui.tracker.loadPattern(song.sequence[idx]);
      ui.tracker.setSelectedRow(0);
    };
  ...
  update() {
    ...
    while (sequence.children.length > 1) sequence.children[1].remove();
    for (var idx = 0; idx < song.sequence.length; idx++) {
      ...
      cell_node.oninput = (e) => {
        ...
        while (value >= song.patterns.length) song.addNewPattern();
  ...
  updatePatternHighlight() {
    ...
    for (var idx = 0; idx < song.sequence.length; idx++) {
      ...
      else sequence.children[idx + 1].classList.remove('highlight');
      if (idx == this.current_index) sequence.children[idx + 1].classList.add('active');
      else sequence.children[idx + 1].classList.remove('active');
    }
  }
}
