...
export default class TrackerUI {
  ...
  constructor() {
    ...
    for (var col = 0; col < 4; col++) {
      var th_node = document.createElement('th');
      if (col == 0) th_node.innerText = 'Duty 1';
      if (col == 1) th_node.innerText = 'Duty 2';
      if (col == 2) th_node.innerText = 'Wave';
      if (col == 3) th_node.innerText = 'Noise';
      th_node.col = col;
      th_node.onclick = (e) => {
        if (player.toggleMute(e.target.col)) e.target.classList.add('muted');
        else e.target.classList.remove('muted');
      };
      header_row.appendChild(th_node);
    }
    ...
    document.getElementById('trackerEffectNibbleHigh').oninput = (e) => {
      this.setEffectParam(e.target.value << 4, 0xf0);
    };
    document.getElementById('trackerEffectNibbleLow').oninput = (e) => {
      this.setEffectParam(e.target.value, 0x0f);
    };
    document.getElementById('trackerEffectByte').oninput = (e) => {
      this.setEffectParam(e.target.value, 0xff);
    };
    document.getElementById('trackerEffectWaveform').onchange = (e) => {
      this.setEffectParam(e.target.selectedIndex << 4, 0xf0);
    };
    document.getElementById('trackerEffectPanningLD1').oninput = (e) => {
      this.setEffectParam(e.target.checked ? 0x01 : 0x00, 0x01);
    };
    document.getElementById('trackerEffectPanningLD2').oninput = (e) => {
      this.setEffectParam(e.target.checked ? 0x02 : 0x00, 0x02);
    };
    document.getElementById('trackerEffectPanningLW').oninput = (e) => {
      this.setEffectParam(e.target.checked ? 0x04 : 0x00, 0x04);
    };
    document.getElementById('trackerEffectPanningLN').oninput = (e) => {
      this.setEffectParam(e.target.checked ? 0x08 : 0x00, 0x08);
    };
    document.getElementById('trackerEffectPanningRD1').oninput = (e) => {
      this.setEffectParam(e.target.checked ? 0x10 : 0x00, 0x10);
    };
    document.getElementById('trackerEffectPanningRD2').oninput = (e) => {
      this.setEffectParam(e.target.checked ? 0x20 : 0x00, 0x20);
    };
    document.getElementById('trackerEffectPanningRW').oninput = (e) => {
      this.setEffectParam(e.target.checked ? 0x40 : 0x00, 0x40);
    };
    document.getElementById('trackerEffectPanningRN').oninput = (e) => {
      this.setEffectParam(e.target.checked ? 0x80 : 0x00, 0x80);
    };
    document.getElementById('trackerEffectDutyCycle').onchange = (e) => {
      this.setEffectParam(e.target.selectedIndex << 6, 0xc0);
    };
    ...
    tracker.onkeydown = (e) => {
      ...
      if (e.code == 'PageUp' || e.code == 'Numpad9') {
        for (var n = 0; n < 16; n++) this.moveUp();
      }
      if (e.code == 'PageDown' || e.code == 'Numpad3') {
        for (var n = 0; n < 16; n++) this.moveDown();
      }
      if (e.code == 'Home' || e.code == 'Numpad7') {
        this.setSelection(0, this.selected_col, this.selected_type);
      }
      if (e.code == 'End' || e.code == 'Numpad1') {
        this.setSelection(63, this.selected_col, this.selected_type);
      }
      if (e.code == 'Insert' || e.code == 'Numpad0') {
        song.patterns[this.pattern_index].splice(this.selected_row, 0, [
          new PatternCell(),
          new PatternCell(),
          new PatternCell(),
          new PatternCell(),
      ...
      if (e.code == 'Backspace') {
        ...
        song.patterns[this.pattern_index].push([
          new PatternCell(),
          new PatternCell(),
          new PatternCell(),
          new PatternCell(),
  ...
  setSelection(row, col, type) {
    ...
    if (this.selected_row !== null) {
      ...
      for (var c of cell.parentElement.parentElement.children)
        for (var cc of c.children) cc.classList.remove('highlight');
  ...
  updateEffectInfo() {
    ...
    else if (c.effectcode === 5)
      info = `Set Left speaker vol to ${c.effectparam >> 4}, Right speaker vol to ${c.effectparam & 0x0f}`;
    ...
    else if (c.effectcode === 10)
      info = `Increase volume by ${c.effectparam >> 4} units, decrease volume by ${c.effectparam & 0x0f}`;
  ...
  setSelectedRow(row) {
    if (this.selected_col !== null) this.setSelection(row, this.selected_col, this.selected_type);
    else this.setSelection(row, 0, 'note');
  }

  getCell(row, col, type) {
    var tracker = document.getElementById('tracker');
    if (type == 'note') return tracker.children[row + 1].children[col + 1].children[0];
    if (type == 'instrument') return tracker.children[row + 1].children[col + 1].children[1];
    if (type == 'effect') return tracker.children[row + 1].children[col + 1].children[2];
  }
