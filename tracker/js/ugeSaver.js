...
export function saveUge(song) {
  ...
  for (var n = 0; n < 15; n++) addInstrument(0, song.duty_instruments[n]);
  for (var n = 0; n < 15; n++) addInstrument(1, song.wave_instruments[n]);
  for (var n = 0; n < 15; n++) addInstrument(2, song.noise_instruments[n]);
  for (var n = 0; n < 16; n++) {
    for (var m = 0; m < 32; m++) addUint8(song.waves[n] ? song.waves[n][m] : 0);
  }
  ...
  for (var track = 0; track < 4; track++) {
    ...
    for (var i of song.sequence) addUint32(i * 4 + track);
